...
load_dotenv()
mongo_url=os.environ.get('MONGO_URL') # the url connexion string
...
def find_thread(sentence:str,tokenizer,model)->str:
    ...
    # print([x for x in collection_thread_embed.find()])
...
def find_thread_message(thread_id:str,sentence:str,tokenizer,model)->str:
    ...
    similarity={}
    test_sentence_embedding=get_sentence_embedding(sentence, tokenizer, model)
    #print(collection_msg.count_documents({'thread_id':thread_id}))
...
def faq(sentence:str,tokenizer,model)->str:
    ...
    message = collection_msg.find_one({'message_id':_id})
    thread = collection_msg.find_one({'message_id':thread_id})
    print(thread['title'])
    print(message['body'])
    return thread['title'],message['body']

if __name__=="__main__":
    id_=faq("Jeune ingénieur", tokenizer, model)
    print(id_)
    exit()